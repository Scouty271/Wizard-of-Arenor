    ...
    private void Update()
    {
        ...
        switch (miniJob)
        {
                ...
                if (GetComponent<Unit>().assignedZoneGroup.jobs.Count > 0 && currJob == null)
                {
                    ...
                    switch (currJob.jobType)
                    {
                        case Job.JobType.createhealingpotion:
                            miniJob = miniJobs.createHealingPotion;
                            break;
                        default:
                            break;
                    }
            ...
            case miniJobs.moveToWorkstation:
                ...
                break;
            ///////////////////////////////////////////////////////////////////////////////////////
            case miniJobs.createHealingPotion:
                workProgress += (workSpeed * Time.deltaTime);
                if (workProgress >= 10)
                {
                    //FindObjectOfType<InventarRohstoffe>().heiltrankAnzahl += 1;

                    FindObjectOfType<Inventory>().addItem(FindObjectOfType<ItemContainer_Arenor>().itemHealingPotion);
